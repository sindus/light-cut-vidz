...
import type { EditorState, TrimSegment } from '../App'
...
export default function ExportModal({ state, onClose }: Props) {
  ...
  // Compute kept segments from cut segments
  const keptSegments = computeKeptSegments(state.cutSegments, state.duration)
...
function computeKeptSegments(cutSegments: TrimSegment[], duration: number): { start: number; end: number }[] {
  if (duration <= 0) return []
  if (cutSegments.length === 0) return [{ start: 0, end: duration }]
