...
export default function Timeline({
  duration, currentTime, cutSegments, isPlaying,
  videoUrl, videoRef, onSeek, onCutSegmentsChange,
}: Props) {
  const { t } = useT()
...
function CutEditor({ seg, duration, onTimeInput, onDelete, onSeek }: CutEditorProps) {
  const { t } = useT()
  const [startVal, setStartVal] = useState(seg ? formatTimePrecise(seg.start) : '')
  const [endVal,   setEndVal]   = useState(seg ? formatTimePrecise(seg.end)   : '')

  // eslint-disable-next-line react-hooks/set-state-in-effect
  useEffect(() => { setStartVal(seg ? formatTimePrecise(seg.start) : '') }, [seg?.start, seg?.id])
  // eslint-disable-next-line react-hooks/set-state-in-effect
  useEffect(() => { setEndVal(seg ? formatTimePrecise(seg.end) : '')     }, [seg?.end,   seg?.id])

  const commit = (edge: 'start' | 'end', raw: string) => {
        ...
        <>
          <div className="cut-editor-field">
            ...
            <input
              ...
              onKeyDown={e => { if (e.key === 'Enter') { commit('start', startVal); (e.target as HTMLInputElement).blur() } }}
          ...
          <div className="cut-editor-field">
            ...
            <input
              ...
              onKeyDown={e => { if (e.key === 'Enter') { commit('end', endVal); (e.target as HTMLInputElement).blur() } }}
              onFocus={e => e.target.select()}
              title={t.cut_format}
            />
...
function mergeSegments(segs: TrimSegment[]): TrimSegment[] {
